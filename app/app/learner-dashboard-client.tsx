...
export type LearnerDashboardLesson = {
  id: string;
  title: string;
  durationS: number | null;
};

export type LearnerDashboardAssignment = {
  id: string;
  module: null | { lessons: LearnerDashboardLesson[] };
  course: null | { modules: { lessons: LearnerDashboardLesson[] }[] };
};

type LearnerDashboardLessonRef = {
  id: string;
  title: string | null;
};
...
export type LearnerDashboardProgress = {
  ...
  lesson: LearnerDashboardLessonRef | null;
};

export type LearnerDashboardBadge = {
  id: string;
  badge: {
    name: string;
  };
...
type LearnerDashboardClientBaseProps = {
  isAdmin: boolean;
};

type LearnerDashboardClientNoOrgState = LearnerDashboardClientBaseProps & {
  status: "no-org";
};

type LearnerDashboardClientErrorState = LearnerDashboardClientBaseProps & {
  status: "error";
};

type LearnerDashboardClientLoadedState = LearnerDashboardClientBaseProps & {
  status: "loaded";
  streak: number;
  assignments: LearnerDashboardAssignment[];
  progresses: LearnerDashboardProgress[];
  badges: LearnerDashboardBadge[];
};

export type LearnerDashboardClientProps =
  | LearnerDashboardClientNoOrgState
  | LearnerDashboardClientErrorState
  | LearnerDashboardClientLoadedState;

type ProgressWithLesson = LearnerDashboardProgress & {
  lesson: NonNullable<LearnerDashboardProgress["lesson"]>;
};
...
export default function LearnerDashboardClient(props: LearnerDashboardClientProps) {
  if (props.status === "no-org") {
    return <LearnerDashboardNoOrgState isAdmin={props.isAdmin} />;
  }

  if (props.status === "error") {
    return <LearnerDashboardErrorState />;
  }

  return <LearnerDashboardLoaded {...props} />;
}

function LearnerDashboardNoOrgState({ isAdmin }: { isAdmin: boolean }) {
  return (
...
function LearnerDashboardErrorState() {
  return (
    <Stack spacing={10} align="flex-start">
      <Stack spacing={3} align="flex-start">
        <Heading size="lg">Your learning</Heading>
...
function LearnerDashboardLoaded(props: LearnerDashboardClientLoadedState) {
  const { assignments, progresses, badges, streak, isAdmin } = props;

  const lessonMap = new Map<string, LearnerDashboardLesson>();
  ...
  const progressesWithLessons = relevantProgresses.filter((item): item is ProgressWithLesson => Boolean(item.lesson));
  ...
  const prioritizedLessons = [...lessons].sort((a, b) => {
    ...
    const orderA = progressA?.isComplete ? 2 : progressA && progressA.watchedSeconds > 0 ? 0 : 1;
    const orderB = progressB?.isComplete ? 2 : progressB && progressB.watchedSeconds > 0 ? 0 : 1;
            ...
            <Stack spacing={4}>
              {queueLessons.length > 0 ? (
                queueLessons.map((lesson) => {
                  ...
                  const helperText = lessonProgress?.isComplete
                    ? "Completed — review anytime."
                    : lessonProgress && lessonProgress.watchedSeconds > 0
                      ? "Resume to keep building momentum."
...
function getLessonCta(progress: LearnerDashboardProgress | undefined): LessonCta {
...
function getLessonProgressPercent(
  lesson: LearnerDashboardLesson,
  progress: LearnerDashboardProgress | undefined,
): number {
  if (!progress) {
    return 0;
  }
...
function formatLessonDuration(durationS: number | null | undefined): string {
  if (!durationS) {
