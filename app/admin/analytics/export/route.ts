...
export async function GET(request: Request) {
  ...
  const dateFromParam =
    url.searchParams.get("dateFrom") ?? url.searchParams.get("start");
  const dateToParam =
    url.searchParams.get("dateTo") ?? url.searchParams.get("end");
  const lessonId = url.searchParams.get("lessonId");
  const startDate = parseDate(dateFromParam);
  const endDate = parseDate(dateToParam, { endOfDay: true });
      ...
      try {
        for (;;) {
          ...
          for (const record of batch) {
            ...
            const percent =
              lessonDuration > 0
                ? Math.min(1, uniqueSeconds / lessonDuration)
                : 0;
            ...
            const sessionCount =
              lessonSessionCountMap.get(record.lessonId) ?? 0;
            ...
            controller.enqueue(
              encoder.encode(`${values.map(escapeCsvValue).join(",")}\n`)
            );
          }
